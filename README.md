...
- A short-form overlay with title, metadata-aware generated description, duration, progress, and compact controls.
...
- `Shared`: small reusable formatting, metadata-derived description, and poster helpers.
...
- Descriptions are generated locally from title and thumbnail URL keywords because the source catalog intentionally does not provide them. This avoids hand-written one-off copy while keeping the prototype offline and scalable.
...
- Add UI tests for rapid swiping, retry, and lifecycle transitions.
- Add adaptive preload sizing based on device memory and network quality.
- Upgrade description generation with on-device Vision/Core ML thumbnail classification if richer captions become product-critical.
...
Included tests cover catalog decoding, metadata-aware generated descriptions, empty/failure view-model states, preload-window behavior at start, middle, end, rapid index changes, background/foreground behavior, continuous scrolling, and 50+ scroll stress.

Latest local verification:

- `xcodebuild -checkFirstLaunchStatus`: exit 0
- `xcrun simctl list runtimes available`: iOS 26.5 available
- `xcodebuild -project Chaptr.xcodeproj -scheme Chaptr -destination 'generic/platform=iOS Simulator' CODE_SIGNING_ALLOWED=NO build`: `BUILD SUCCEEDED`
- `xcodebuild test -project Chaptr.xcodeproj -scheme Chaptr -destination 'platform=iOS Simulator,name=iPhone 17,OS=26.5' CODE_SIGNING_ALLOWED=NO`: `TEST SUCCEEDED`, 37 tests, 0 failures
